...
Deno.serve(
  handle(async (req) => {
    ...
    const { data: room, error: roomErr } = await admin.from("rooms").insert({ session_id: session.id, code: randomCode("R"), name }).select("id").single<{ id: string }>();
    ...
    return json({ ok: true, world, room: { id: room.id, name } });
  }),
);
